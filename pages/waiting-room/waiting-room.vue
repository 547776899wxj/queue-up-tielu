<template>
	<view class="content"  @longpress="open" @click="open">
		...
		<view class="info">
			<view class="info-patient" v-for="(item,index) in pageData" :key="index">
	...
	methods: {
		...
		page(){
			...
			if(this.pageNumber>this.pageNewNumber){
				setTimeout(() => {
					this.pageData = this.data.slice(this.pageNewNumber*7,(this.pageNewNumber*7)+7);
					this.pageNewNumber++;
					this.page();
				}, 10000);
				console.log("page1");
		...
		onDone(data){
			...
			setTimeout(() => {
				...
				if(this.voiceData.length>0){
					this.voiceQueue()
				}else if(this.voicePlayNumber==0 && this.data.length<=7){
					this.init();
				}
			}, date);
